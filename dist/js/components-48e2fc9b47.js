...
table.prototype.loadFromTemplateJson = function (DataSource, Options) {
  if (typeof DataSource == "string") {
    ...
    if (c.length - 1 == c.lastIndexOf('json')) return this.load(DataSource);
  }
  var template = {
    "tablename": "template",
    "hasHeader": false,
...
table.prototype.new = function (Obj, header) {
  ...
  var Json = Obj;
  if (!Obj.hasHeader) Json["data"].pop();
...
table.prototype.update = function (rowKey, row) {
  ...
  if (row instanceof Array) set[rowKey] = row;else {
...
table_row.prototype.rowAddCard = function (color) {
  ...
  for (var i in this.keyArr) {
    ...
    props[this.Headers[i]] = $(tds[i]).html(); //展示没有必要XSS,控制写入时就行
    //遍历keyArr,找到key对应的数据，填入
    // debugger
    //ie没有includes 只能用search
    if (this.keyArr[i].search("hide") >= 0) continue;
...
table_row.prototype.buildCard = function () {
  ...
  for (var headertitle in cardHeader) {
    ...
    if (headertextcontent.length > 20) headertextcontent = headertextcontent.substring(0, 20) + "...";
    ...
    if (!(c = regxRule[category])) {
      ...
      if ('function' == typeof c["regx"]) {
        if (limit) {
          ...
          var max = "";
          var MinAndMax = "";
          if (limitList.length > 2) throw 'only two integers required,Max and Min';else if (limitList.length == 2) {
            try {
...
var PurchaseItem = {
  ...
  autoComplate: function autoComplate(PI) {
    ...
    var PInfoSet = 'object' == (typeof PI === 'undefined' ? 'undefined' : _typeof(PI)) ? PI : apiConfig.purchaseitem.Get(PI); //查出改PI详情
  ...
  event: {
    ...
    append: function append() {
      //是否fields全为空
      if (isAllPRTypeFormFieldEmpty("#PruchaseItem_form")) return;
      //
      var arr = [];
...
var PurchaseRequisition = {
  ...
  destory: function destory() {
    ...
    if (window._target) {
      if (window._target.PR) window._target.PR = null;
      window._target = null;
    }
    window._operation = null;
  ...
  init: function init(PRid) {
    ...
    if (!window.target.PR) window.target.PR = {};
    if (PRid) {
  ...
  destory: function destory() {
    ...
    if (window.target.PR) window.target.PR = null;
    if (window.target) window.target = null;
  },
  autoComplate: function autoComplate(PRid) {
...
function setToArray(set, arrOrder) {
  var arr = [];
  if (arrOrder) for (var i = 0; i < arrOrder.length; i++) {
...
function autoComplateInfo(infoSet, form, prefix) {
  ...
  for (var i in infoSet) {
    var val = infoSet[i];
    if ("" != prefix && undefined != prefix) i = prefix + i;
